...
ggsave(file.path(fig_dir, paste0("Fig5_volcano_mutation_cox_cachexia_DFCI_", THRESHOLD, "_", DATE_STAMP, ".pdf")),
       volcano_dfci, width = 2.6, height = 3, units = "in", device = "pdf")
...
clean_dfci_label <- function(dt) {
  ...
  dt[, cov_label := gsub("STAGE_CDM_DERIVED_GRANULAR_", "Stage ", cov_label)]
                  ...
                  fifelse(covariate %like% "ANCESTRY_LABEL", "Ancestry",
                   fifelse(covariate %like% "age_at_diagnosis_binned", "Demographic",
                    fifelse(covariate %in% gene_set, "Gene",
                     fifelse(covariate == "CVR_TMB_SCORE" | covariate %like% "^STAGE_CDM_DERIVED_GRANULAR",
...
if (file.exists(nsclc_fp)) {
  ...
  ordered_labels <- c("Unspecified", "Primary", "Metastatic Recurrence",
                       "Male",
                       "TP53",
                       "CVR TMB Score", "Stage 2", "Stage 3", "Stage 4",
                       "Age <40", "Age 50-59", "Age 60-69", "Age 70-79", "Age >=80")
  ...
  ggsave(file.path(fig_dir, paste0("Fig5_multivariate_nsclc_DFCI_", DATE_STAMP, ".pdf")),
         p_nsclc, width = 2.6, height = 3.6, units = "in", device = "pdf")
