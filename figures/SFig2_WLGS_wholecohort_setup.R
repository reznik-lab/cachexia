...
# ---- baseline (first-observed) smoothed BMI per patient, for those with no valid episode ----
...
# ---- Fearon-style WLGS grade + 5x5 bins, applied uniformly to any (start_bmi, weight_loss) row ----
...
# ---- episode-level graded table (one row per valid episode, retains start_day) ----
# used by the WLGfirst time-dependent Cox analysis (grade of a patient's FIRST episode)
eps_wlgs_episode <- assign_wlgs(eps[has_cachexia_valid_edemaQC == 1, .(MRN, start_day, start_bmi, weight_loss)])

# ---- whole-cohort row table: real episodes keep their own start_bmi/weight_loss;      ----
# ---- patients with no valid episode are "weight stable" -> baseline BMI, 0% loss      ----
...
# ---- collapse to one row per patient: their single worst (highest-grade) state ----
# tie-break: among a patient's rows tied on grade, keep the one with the larger weight_loss
