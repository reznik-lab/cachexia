...
spans_fp <- if (WL_LABEL == "WL5_BMIlt20") {
  file.path(REV_RESULTS, paste0("spans_fearon_WL5_BMIlt20_", DATE_STAMP, ".csv"))
} else {
  file.path(REV_RESULTS, paste0("spans_fearon_", WL_LABEL, "_", DATE_STAMP, ".csv"))
}
...
labs[, MRN := as.integer(MRN)]
spans[, MRN := as.integer(MRN)]

labs[, Date := as.IDate(Date)]
msk_clin[, anchor_final := as.IDate(anchor_final)]
labs <- merge(labs, msk_clin[, .(MRN, anchor_final, CANCER_TYPE_DETAILED, GENDER,
                                  AGE_AT_ANCHOR_YEARS, AGE_AT_ANCHOR, STAGE_CDM_DERIVED)],
              by = "MRN", all.x = TRUE)
labs <- labs[!is.na(anchor_final) & !is.na(Date)]
labs[, days_since_diagnosis := as.integer(Date - anchor_final)]

spans_keep <- unique(spans[, .(MRN, start_day, end_day, span)])
setkey(spans_keep, MRN); setkey(labs, MRN)
sp <- merge(spans_keep, labs, by = "MRN", allow.cartesian = TRUE)
sp <- sp[!is.na(days_since_diagnosis) & days_since_diagnosis >= start_day & days_since_diagnosis <= end_day]
...
ggsave(file.path(OUT_PLOT_DIR, paste0("derived_labs_summary_", OUT_TAG, "_", DATE_STAMP, ".pdf")),
       p_bar, width = 4, height = 2.7, device = "pdf")
