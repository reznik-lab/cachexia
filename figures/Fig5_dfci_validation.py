...
def stage_multivariate(cox_df=None, all_df=None):
    ...
    for cancer_type_dfci, dfg in all_df.groupby("cancer_type"):
        ...
        df_dummy_src = df[variables].copy()
        if "STAGE_CDM_DERIVED_GRANULAR" in df_dummy_src.columns:
            df_dummy_src["STAGE_CDM_DERIVED_GRANULAR"] = (
                df_dummy_src["STAGE_CDM_DERIVED_GRANULAR"].astype(int).astype(str)
            )
        df_design = pd.get_dummies(df_dummy_src, drop_first=True)
