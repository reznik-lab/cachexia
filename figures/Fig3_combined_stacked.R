...
build_combined <- function(wl_label, out_dir, title_top, title_bottom, outname) {
  ...
  # x-axis order/set shared between the two panels: driven by early-onset prevalence (descending)
  ...
  ggsave(file.path(out_dir, outname), plot = p_combined, width = 4.8, height = 5.0, dpi = 300, device = pdf)
  p_combined
}

# Fig3B (main): consensus / WL5+modifier
